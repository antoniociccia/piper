...
> **Recording coming in a follow-up commit.** The reproducible take is
> generated by `bash scripts/record-demo.sh` and lands at
> `docs/assets/demo.gif`. Until then, here's the script anyone can run
> locally in under a minute.

The repo ships a self-contained Docker host — a fake "Orderly" production
box with planted anomalies (OOM redis cascading into a dead worker,
suspicious traffic in nginx logs from a fake `Example-Audit-Bot`). Bring it
up, point PIPER at it, watch the agent narrate a real postmortem:

```bash
# 1. Spin up the demo host + add it to PIPER.
bash demo/setup.sh
docker compose -f demo/docker-compose.yml up -d

# 2. Run PIPER against it.
bun dev
...
> **Reproducible recording.** [`scripts/record-demo.sh`](scripts/record-demo.sh)
> wraps `asciinema` + `agg` around a live PIPER session and writes
> `docs/assets/demo.cast` (JSON, replays in any terminal via `asciinema play`)
> and `docs/assets/demo.gif` (README-embed-ready). Recording cookbook +
> pre-warm checklist at [`demo/RECORDING.md`](demo/RECORDING.md).
